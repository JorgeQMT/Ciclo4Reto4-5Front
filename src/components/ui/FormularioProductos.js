...
const FormularioProductos = ({producto}) => {
    ...
    const {reference, brand, name , price, photography, category,quantity } = producto;

    const [ productos, guardarProductos] = useState([]);

    fetch("http://168.138.249.154:8080/api/cosmetics/all")
    .then((res) => res.json())
    .then((data) => {
        //console.log(data);
    ...
    const actualizarProducto = reference =>{{
        productos.map( producto =>(
                <ActualizarProducto key={producto.reference} producto={producto} /> 
            ))
        }
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try{
                    ...
                    fetch(`http://168.138.249.154:8080/api/cosmetics/${reference}`,{
                        method: "DELETE",
                        headers: {
            ...
            <div className="p-5 shadow-md bg-white">
                <div className="lg:flex">
                    ...
                    <div className="lg:w-7/12 xl:w-9/12 pl-5">
                        ...
                        <button onClick={ () => borrarProducto(producto.reference)} type="submit" className=" bg-red-800 hover:bg-blue-700, inline-block mb-5 p-2 text-white uppercase font-bold"> 
                            Eliminar Producto
                        </button>
                        <br/>
                        <Link to={`/actualizar-producto/${producto.reference}`} className="  bg-yellow-800 hover:bg-blue-700, inline-block mb-5 p-2 text-white uppercase font-bold">
                            Actualizar Producto
                        </Link>
                    </div>
                </div>
